    ...
    <Draggable draggableId={recipe.id} index={index}>
      {(provided, snapshot) => (
        <div
          ...
          className={`group cursor-grab active:cursor-grabbing p-[13px] rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm hover:border-primary flex items-center gap-[12px] mb-3 ${
        ...
        >
          <div className="flex-1 min-w-0">
            <p className="text-[14px] font-semibold text-slate-900 dark:text-slate-100 leading-[20px] truncate">{recipe.name}</p>
          ...
          <span className="material-symbols-outlined text-slate-300 group-hover:text-primary shrink-0">drag_indicator</span>
        </div>
      )}
    </Draggable>
  );
